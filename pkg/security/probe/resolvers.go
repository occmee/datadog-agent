...
func (r *Resolvers) Start() error {
	...
	if r.procCacheMap == nil {
		return fmt.Errorf("proc_cache BPF_HASH table doesn't exist")
	}

	// Select the in-kernel pid <-> cookie cache
	r.pidCookieMap = r.probe.Table("pid_cookie")
	if r.pidCookieMap == nil {
		return fmt.Errorf("pid_cookie BPF_HASH table doesn't exist")
	}
...
func (r *Resolvers) Snapshot(retry int) error {
	...
	if r.inodeNumlowerMap == nil {
		return fmt.Errorf("inode_numlower BPF_HASH table doesn't exist")
	}
...
func NewResolvers(probe *ebpf.Probe) (*Resolvers, error) {
